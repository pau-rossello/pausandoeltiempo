    ...
    if (typeof gsap !== 'undefined') {
        ...
        if (revealText) {
            ...
            revealText.innerHTML = words.map(word => {
                return `<span class="inline-block whitespace-nowrap mr-[0.25em]">` + 
                    word.split('').map(char => 
                        `<span class="opacity-20 transition-opacity duration-300">${char}</span>`
                    ).join('') + 
                `</span>`;
            }).join('');

            const chars = revealText.querySelectorAll('span > span');
            ...
            gsap.to(chars, {
                scrollTrigger: {
                    trigger: "#philosophy",
                    start: "top top",
                    end: "+=150%", // Ends halfway through the 300vh section
                    scrub: 1,
                },
                opacity: 1,
                stagger: 0.1,
        ...
        phiPhotos.forEach((photo, i) => {
            ...
            gsap.fromTo(photo, {
                ...
                opacity: 0.4, // Keep opacity low
                filter: "blur(2px)", // Slightly more focused than before
                y: -100 * speed * 5, // Parallax movement
                ease: "power2.out"
            });
        });
    }
